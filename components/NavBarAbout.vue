...
<template>
  <nav :class="{ scrolled: !atTopOfPage }" class="w-full bg-[#673F7B]">
    ...
    >
      <NuxtLink to="/">
        <div class="flex gap-1 items-center">
          ...
          <img
            ...
            alt=""
            class="w-14 h-14 md:w-20 md:h-20 object-cover block"
            :class="{ scrolled: !atTopOfPage }"
          />
          <h2
            class="text-base font-semibold md:text-xl md:font-medium text-gray-50"
            :class="{ scrolled: !atTopOfPage }"
          >
            Invictus <br class="block md:hidden" />
            Development Africa
          </h2>
        </div>
      </NuxtLink>
      <div>
        <ul
          class="flex gap-3 text-sm md:font-normal md:gap-6 items-center font-medium text-gray-50 uppercase"
          :class="{ scrolled: !atTopOfPage }"
        >
          <NuxtLink
            to="/"
