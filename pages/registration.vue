...
const farmerRules = computed(() => {
  return {
    name: {
      ...
      minLength: helpers.withMessage('A name must have at least three characters.', minLength)
    },
    email: {
...
const farmParticularsRules = computed(() => {
  return {
    farmingPractice: {
    required: helpers.withMessage('What farm practices are you practicing', required)
  },
  newCrop: {
  ...
  // },
  yearsOfExperience: {
    required: helpers.withMessage('Enter your farming experience in years', required)
  },
  farmSize: {
    required: helpers.withMessage('Please enter the size of your farm', required)
  },
  }
}) 
...
const countryValid = computed(() => w$?.selectedCountry?.$dirty && !w$?.selectedCountry?.required)
const stateValid = computed(() => w$?.selectedState?.$dirty && !w$?.selectedState?.required)
const cityValid = computed(() => w$?.selectedCity?.$dirty && !w$?.selectedCity?.required)
...
<template>
  <div class="absolute inset-0 z-0 bg-purple-50 min-h-screen w-full pt-[100px]">
    <div class="max-w-6xl mx-auto">
      <div class="mx-5 md:mx-10 lg:mx-40 pt-2 bg-gray-50 rounded-md shadow-md">
        <div class="flex p-5">
          <div class="hidden md:block md:w-3/12 mx-4 p-4">
            <div class="flex cursor-pointer text-purple-800 items-center" @click.prevent="currentStep(1)">
              <div
              ...
              'text-purple-800 border-purple-800': step === 1 || step1Complete,
            }">
                <Icon v-if="!step1Complete" name="ic:baseline-person-add-alt-1" class="w-5 h-5 " :class="{
              'text-gray-500': step !== 1,
              'text-purple-800 border-purple-800': step === 1 || step1Complete,
            }" />
                <Icon v-else name="heroicons:check-16-solid" class="w-7 h-7 " :class="{
              'text-gray-500': step !== 1,
              'text-purple-800 border-purple-800': step === 1 || step1Complete,
            }" />
              </div>
              <div class="ml-5">
                <h5 class="uppercase font-medium text-xs" :class="{
              'text-gray-500 ': step !== 1,
            ...
            <div class="border-l-2 h-16 transition duration-500 ease-in-out ml-6" :class="{
              'text-gray-500': step !== 1,
              'text-purple-800 border-purple-800': step === 1 || step1Complete,
            }"></div>
            ...
            <!-- Second Step -->
            <div class="flex cursor-pointer items-center" @click.prevent="currentStep(2)">
              <div
                ...
                :class="{
              'text-': step !== 2,
              'text-purple-800 border-purple-800': step === 2 || step2Complete,
            }">
                <Icon v-if="!step2Complete" name="heroicons:map-pin-16-solid" class="w-5 h-5 " :class="{
            ...
            <!-- STEP 3 Details -->
            <div class="flex cursor-pointer text-purple-800 items-center" @click.prevent="currentStep(3)">
              <div
                class="flex flex-shrink-0 items-center justify-center rounded-full transition duration-500 ease-in-out h-12 w-12 py-3 border-2 hover:scale-105"
                :class="{
              'text-gray-500 bg-white': step !== 3,
              'text-purple-800 border-purple-800': step === 3 || registrationComplete,
            }">

                <Icon v-if="!step3Complete" name="heroicons:clipboard-document-list" class="w-5 h-5 " :class="{
              'text-gray-500': step !== 3,
              'text-purple-800 border-purple-800': step === 3 || step3Complete,
            }" />
                <Icon v-else name="heroicons:check-16-solid" class="w-7 h-7 " :class="{
              'text-gray-500': step !== 3,
              'text-purple-800 border-purple-800': step === 3 || step3Complete,
          ...
          <div class="w-full md:w-9/12 bg-white p-5 rounded-md shadow-md">
            <div v-if="step === 1">
              <div class="flex flex-col overflow-y-auto md:flex-row">
                <div class="flex items-center justify-center md:px-10 md:py-4 sm:p-4 md:w-full">
                  <div class="w-full">
                    <h1 class="mb-4 text-xl flex gap-5 items-center font-semibold">
                      Personal Information <span class="block md:hidden font-medium text-base">(1/3)</span>
                    </h1>
                    <div class="w-full space-y-1">
                      <div>
                        <label for="name" class="mt-4 text-xs">
                          Full Name
                          <input id="name" v-model="details.name" type="text" @change="v$.name.$touch" :class="{
              ...
              v$.name.$errors[0].$message
                      ...
                      <div>
                        <label for="email" class="mt-4 text-xs">
                          Email Address
                          <input id="email" v-model="details.email" type="email" @change="v$.email.$touch" :class="{
              ...
              v$.email.$errors[0].$message
                      ...
                      <div>
                        <label for="email" class="mt-4 text-xs">
                          ...
                          <input id="phone" v-model="details.phone" type="text" @change="v$.phone.$touch" :class="{
              'border-red-500 focus:border-red-500': v$.phone.$error,
              'border-[#42d392] ': !v$.phone.$invalid,
            }" />
                          <div>
                            <span class="text-xs text-red-500" v-if="v$.phone.$error">{{
            ...
            <div v-if="step === 2">
              <div class="flex flex-col overflow-y-auto md:flex-row">
                <div class="flex items-center justify-center md:px-10 md:py-4 sm:p-4 md:w-full">
                  <div class="w-full">
                    <h1 class="mb-4 text-xl flex items-center gap-5 font-semibold">
                      Farm and Farmer Location <span class="block md:hidden font-medium text-base">(2/3)</span>
                    </h1>
                    <div class="w-full space-y-3">
                      <div>
                        <label for="country" class="mt-4 text-xs">
                          Country
                          <select id="country" v-model="selectedCountry" name="country" required @change="w$?.selectedCountry?.$touch()">   
    ...
    'border-[#42d392] ': !w$.selectedCountry.$invalid,
              ...
              country.label
                          ...
                          <div>
                            <div v-if="!countryValid" class="error-message text-gray-400">*Required</div>
              ...
              w$.selectedCountry.$errors[0].$message
                      ...
                      <div>
                        <label for="state" class="mt-4 text-xs">
                          ...
                          <select id="state" v-model="selectedState" required name="state" :disabled="!selectedCountry" @change="w$?.selectedCountry?.$touch()" >
    ...
    'border-[#42d392] ': !w$.selectedState.$invalid,
              ...
              state.label
                          ...
                          <div>
                            <div v-if="!stateValid" class="error-message text-gray-400">*Required</div>
              ...
              w$.selectedState.$errors[0].$message
                      ...
                      <div>
                        <label for="city" class="mt-4 text-xs">
                          ...
                          <select id="city" v-model="selectedCity" required name="city" :disabled="!selectedState" @change="w$?.selectedCountry?.$touch()">
    ...
    'border-[#42d392] ': !w$.selectedCity.$invalid,
              ...
              city.label
                          ...
                          <div>
                            <div v-if="!cityValid" class="error-message text-gray-400">*Required</div>
              ...
              w$.selectedCity.$errors[0].$message
                      ...
                      <div>
                        <label for="specificLocation" class="mt-4 text-xs">
                          ...
                          <input id="specificLocation" v-model="details.specificLocation" type="text" name="" />
                        </label>
                      </div>
                    </div>
            ...
            <!-- THIRD STEP ************ -->
            <div v-if="step === 3">
              <div v-if="!registrationComplete" class="flex flex-col overflow-y-auto md:flex-row">
                <div class="flex items-center justify-center md:px-10 md:py-4 sm:p-4 md:w-full">
                  <div class="w-full">
                    <h1 class="mb-4 text-xl flex items-center gap-5 font-semibold">
                      Farm Particulars and Production <span class="block md:hidden font-medium text-base">(3/3)</span>
                    </h1>
                    <div class="w-full space-y-1">
                      <div class="mb-2 text-sm">
                        <label class="block mb-2">Farming Practices:</label>
                        <div class="mb-2">
                          <input type="radio" id="organic" value="Organic" v-model="details.farmingPractice" @change="x$.farmingPractice.$touch" :class="{
              'border-red-500 focus:border-red-500': x$.farmingPractice.$error,
              'border-[#42d392] ': !x$.farmingPractice.$invalid,
            }">
                          <label for="organic" class="ml-2">Organic</label>
                          <div>
                                      <span class="text-xs text-red-500" v-if="x$.farmingPractice.$error">{{
                      ...
                      <div class="">
                        <label class="mt-4 text-xs">Primary Crops you Plant in your Farm:
              ...
              'border-[#42d392] ': !x$.newCrop.$invalid,
                            ...
                            <Icon @click="addNewCrop" name="heroicons:plus-circle" color=""
              ...
              x$.newCrop.$errors[0].$message
                          ...
                          <ul class="flex flex-wrap gap-3 mt-3">
                            <li v-for="(crop, index) in details.primaryCrops" :key="index"
                              class="py-[2px] px-2 bg-purple-800 rounded-md text-white font-medium text-sm">{{ crop }}
                              <Icon @click="removeCrop(index)" name="heroicons:x-mark-20-solid"
                      ...
                      <div>
                        <label for="farmSize" class="mt-4 text-xs">
                          Farm Size (Acres)
                          <input id="farmSize" v-model="details.farmSize" type="number" min="0" name="" newCrop @change="x$.farmSize.$touch" :class="{
              'border-red-500 focus:border-red-500': x$.farmSize.$error,
              'border-[#42d392] ': !x$.farmSize.$invalid,
            }" />
                        </label>
                        <div>
                            <span class="text-xs text-red-500" v-if="x$.farmSize.$error">{{
              x$.farmSize.$errors[0].$message
            }}</span>
                          </div>
                      </div>
                      <div>
                        <label for="farmingExperience" class="mt-4 text-xs">
                          ...
                          <input id="farmingExperience" v-model="details.yearsOfExperience" type="number" min="0"
                            name="" @change="x$.yearsOfExperience.$touch" :class="{
              'border-red-500 focus:border-red-500': x$.yearsOfExperience.$error,
              'border-[#42d392] ': !x$.yearsOfExperience.$invalid,
            }" />
                        </label>
                        <div>
                            <span class="text-xs text-red-500" v-if="x$.yearsOfExperience.$error">{{
              x$.yearsOfExperience.$errors[0].$message
                    ...
                    <hr class="my-8" />
                    <UButton :loading="isLoading" loading-icon="i-eos-icons-loading" @click="register" size="md"
                      color="purple" block>
                      Register
                    </UButton>
                  </div>
